            ...
            <>
                          ...
                          <ul>
                            {h1.children.map((h2, j) => (
                              <li key={h2.id} style={{ marginLeft: '15px' }}>
                                <button onClick={() => handleScrollToHeading(h2.id)}>
                                  {i + 1}.{j + 1} {h2.text}
                                </button>
                                {h2.children.length > 0 && (
                                  <ul>
                                    {h2.children.map((h3, k) => (
                                      <li key={h3.id} style={{ marginLeft: '30px' }}>
                                        <button onClick={() => handleScrollToHeading(h3.id)}>
                                          {i + 1}.{j + 1}.{k + 1} {h3.text}
                                        </button>
                                      </li>
                                    ))}
              ...
              <div className="post-meta">
                <div className="meta-left">
                  ...
                  <p>on {post.createdAt ? new Date(post.createdAt).toLocaleDateString() : 'N/A'}</p>
                </div>
              ...
              {post.owner && (
                <div className="post-actions">
                  ...
                  <button className="post-view__delete-button" onClick={handleDeleteButtonClick}>
                    🗑️ Delete
                  </button>
                </div>
              )}
